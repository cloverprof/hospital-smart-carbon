...
// 院区选择 / 数据日期 / 全屏 / 返回登录 / 重置演示。
// 3D 环绕开关：3D 方案未通过资产可行性门槛（.max 无合法转换路径），按提示词 10.2 不显示
// 可开启开关，仅在含场景页面显示禁用态说明（tooltip）。
import { Box, ChevronDown, Expand, LogOut, MoreHorizontal, RotateCcw, Shrink } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { NavLink, useNavigate } from "react-router-dom";
import { campuses } from "../data/buildings";
import { PRODUCT_NAME, demoAsOfDate } from "../data/config";
...
export function TopBar({ cockpitName, withScene = false }: { cockpitName?: string; withScene?: boolean }) {
  ...
  const { role, setRole, allPermissions, setAllPermissions, campus, setCampus, resetDemo } = useDemoStore();
  ...
  return (
    <header className="topbar">
      <div className="tb-brand">
        <b>{PRODUCT_NAME}</b>
        <span>{cockpitName ?? "医院智慧能碳管理平台"}</span>
      </div>
      ...
      <div className="tb-group">
        ...
        <select className="pf-select" aria-label="演示角色切换" value={role} onChange={(e) => setRole(e.target.value as RoleId)}>
        ...
        <select className="pf-select" aria-label="院区选择" value={campus} onChange={(e) => setCampus(e.target.value as typeof campus)}>
          {campuses.map((c) => (
      ...
      <div className="tb-group">
        ...
        <ChevronDown size={0} style={{ display: "none" }} />
      </div>
    </header>
  );
}
